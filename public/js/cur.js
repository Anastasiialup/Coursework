document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch(apiUrl)
        ...
        .catch(error => {
            ...
            result.textContent = 'Не вдалося завантажити дані про обмінні курси.';
        });
    ...
    convertButton.addEventListener('click', () => {
        ...
        if (isNaN(amount)) {
            result.textContent = 'Будь ласка, введіть дійсну суму.';
            return;
        }
        ...
        fetch(`https://v6.exchangerate-api.com/v6/${apiKey}/pair/${from}/${to}`)
            ...
            .then(data => {
                ...
                result.textContent = `${amount} ${from} дорівнює ${convertedAmount} ${to}`;
            })
            .catch(error => {
                console.error('Error fetching the conversion rate:', error);
                result.textContent = 'Не вдалося конвертувати валюту.';
            });
    });
});